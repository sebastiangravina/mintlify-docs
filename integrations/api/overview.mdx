...
title: How to use API integrations in Release0
description: Learn how to execute Release0 agents via HTTP requests for integration with external services and systems.
---
...
2. To start the chat, send a POST request to `https://release0.com/api/v1/agents/<Agent ID>/preview/start`.  
   The first response will contain a `sessionId` that you will need for subsequent requests.
3. To send replies, send POST requests to `https://release0.com/api/v1/sessions/<session ID>/continue`  
...
1. To start the chat, send a POST request to `https://release0.com/api/v1/agents/<public ID>/start`.  
   The first response will contain a `sessionId` that you will need for subsequent requests.
2. To send replies, send POST requests to `https://release0.com/api/v1/sessions/<session ID>/continue`  
   Use the following JSON body:
