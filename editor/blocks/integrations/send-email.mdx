...
> ⚠️ **Important:**
Make sure you fill in the  **From:**  of the destiantion email **before** configuring the SMTP settings. This is the email address where you want to receive notifications. You can use a variable to dynamically set the destination email based on user input.
If this is not configured before setting up the SMTP settings, the setup process will fail since the test conenction email sill not be triggered.
<Frame style={{ maxWidth: '400px' }}>
...
4. Make sure you fill in the  **From:**  of the destiantion email **before** configuring the SMTP settings. Otherwise, you will receceive an error message '*Invalid Configuration. We couldn't send the latest email with your configuration*'.
