...
There are cases where you might want to store sensitive information, such as passwords or personal identifiers. To ensure these variables are not stored in the results table, prefix the variable name with `secure_`. For example:

```plaintext
secure_Password
```
This variable will not be saved in the results table, but you can still use it in your flow logic.
